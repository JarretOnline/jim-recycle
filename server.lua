...
QBCore.Functions.CreateUseableItem("wallet", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local ranomizer = math.random(100,200)
    Player.Functions.RemoveItem("wallet", 1, false, info)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["wallet"], "remove", 1)
	Player.Functions.AddMoney("cash", tonumber(ranomizer))
end)
...
RegisterServerEvent("jim-recycle:TradeItems", function(data)
	...
	elseif data == 2 then
		if Player.Functions.GetItemByName('recyclablematerial') ~= nil and Player.Functions.GetItemByName('recyclablematerial').amount >= 100 then
			...
			TriggerClientEvent('QBCore:Notify', src, "MISSING ITEMS!")
		end
    end
end)
